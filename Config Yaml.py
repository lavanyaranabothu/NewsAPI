...
import yaml
def getyaml(key,value):
    file=open("config.yaml")
    data=yaml.safe_load(file)
    return data.get(key,value)

def everything(user_choice,basedon,Fromdate=None,Todate=None,Records=None):
    url=f'{getyaml('everything','url')}'
    dict={"q":user_choice,'apikey':getyaml('everything','apikey')}
    ...
    if response.status_code == 200:
        ...
        print(tabulate.tabulate(outer_list, headers=('everything','headers'),tablefmt="simple_grid"))
    else: print("Invalid Data")
def headlines(country,category,Records=None):
    url=f'{getyaml('headlines',"url")}'
    mydict={"q":"country","apikey":getyaml("headlines","apikey")}
    ...
    if response.status_code == 200:
        ...
        for ele in output["articles"]:
            inner_list = (ele["author"], ele["title"],ele["publishedAt"])
            outer_list.append(inner_list)
        print(tabulate.tabulate(outer_list,headers=('headlines','headers'),tablefmt="simple_grid"))
    else:
        print(" Invalid Data")
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            basedon = getyaml("searchdict",choice)
            ...
            print(" you have selected Headlines End point : ")
            ...
            while True:
                ...
                if record in ["Y", "y"]:
                    ...
                    print(" select either yes or no ")
            headlines(country,category,Records)
