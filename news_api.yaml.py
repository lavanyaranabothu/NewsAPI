...
file= open('news_api.yaml')
data= yaml.safe_load(file)

def everything(user_choice,basedon,Fromdate=None,Todate=None,Records=None):
    url=data["everything"]["url"]
    dict={"q":user_choice,'apikey':data["everything"]["apikey"]}
    ...
    response = requests.get(data["everything"]["url"],params=dict)
    ...
    #     print(tabulate.tabulate(outer_list, headers=(data["everything"]["headers"]),tablefmt="simple_grid"))
    # else:
    #     print(" Invalid Data")
def headlines(country,category,Records=None):
    url=data["headlines"]["url"]
    mydict={"q":"country","apikey":data["headlines"]["apikey"]}
    ...
    print(url)
    response=requests.get(data["headlines"]["url"],params=mydict)
    print(response.request.url)
    if response.status_code == 200:
        output = response.json()
        if len(output) > 0:
        ...
        for ele in output["articles"]:
            inner_list = (ele["author"], ele["title"],ele["publishedAt"])
            outer_list.append(inner_list)
        print(tabulate.tabulate(outer_list,headers=(data["headlines"]["headers"]),tablefmt="simple_grid"))
    else:
        print(" Invalid Data")
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            user_choice = input(" Enter the search choice  : ")
            ...
            basedon = (data["search_dict"],choice)
            ...
            print(" you have selected Headlined End point : ")
            ...
            print('1.United Arab Emirates', '2.Argentina', '3.Austria', '4.Australia', '5.Belgium', '6.Bulgaria',
                  ...
                  '54.South Africa', sep="\n")
            choice = input(" enter your country choice : ")
